...
def _to_path(p):
    """Return a pathlib.Path no matter what is passed in."""
...
class VesselsSemanticFramework(BaseFramework):
    ...
    def __init__(self, parent_path: str,  num_classes: int, start_model: str = None):
        ...
        # Always keep internal paths as Path objects for '/'
        # BaseFramework usually sets these, but we coerce defensively
        if hasattr(self, 'patch_path'):
            self.patch_path = _to_path(self.patch_path)
        if hasattr(self, 'model_path'):
            self.model_path = _to_path(self.model_path)
    ...
    def train(self, updater=None):
        ...
        patch_path = _to_path(self.patch_path)
        ...
        torch.save(self.model, _to_path(self.model_path) / self.model_name)

    def predict(self, image):
        # Be robust to machines without CUDA
    ...
    def load_model_from_disk(self, model_name):
        # Allow loading on CPU-only systems; weights_only kept for torch>=2.0 compatibility
        map_location = 'cuda' if torch.cuda.is_available() else 'cpu'
        self.model = torch.load(model_name, map_location=map_location, weights_only=False)
